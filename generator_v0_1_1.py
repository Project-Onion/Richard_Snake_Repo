...
def deepnn(x, keep_prob):
    ...
    with tf.name_scope('conv1'):
        ...
        W_conv1 = weight_variable([5, 5, 3, 32]) #feature size 5x5 to have 46x46 image after convolution
    ...
    # with tf.name_scope('conv2'):
    #     W_conv3 = weight_variable([4, 4, 32, 64]) #feature size 4x4 to have 20x20 image after convolution
    ...
    with tf.name_scope('fc1'):
        W_fc1 = weight_variable([20 * 20 * 64, 4096]) #4096 = first power of 2 larger than 2500 (=50x50)
        b_fc1 = bias_variable([4096])

        # h_pool2_flat = tf.reshape(h_conv2, [-1, 10 * 10 * 64])
        h_conv3_flat = tf.reshape(h_conv2, [-1, 20 * 20 * 64])
