...
def main(_):
    ...
    amountOfMiniBatchFilesToTrain = 40#90
    amountOfMiniBatchFilesToValidate = 1
    amountOfMiniBatchFilesToTest = 10#30
...
#                                   learning_rate: starting_learning_rate})
            ...
            if i%3==0:
                print("minibatch file: " + str(i) + " started validation using data files number: " +str(dataFileNumber) + ". time passed: "+ str(time.time()-startTime))
                sys.stdout.flush()
                sumOfValidations = 0
                validationBatchData = [np.load(fileLocation + "validationData/" + 'validationDataBoards' + str(1) + ".npy"), np.load(fileLocation + "validationData/" + 'validationDataMoves' + str(1) + ".npy")]
                amountOfValidations = 0
                validation_confusion = np.zeros([3,3])
                for batchStartIndex in range(0, len(validationBatchData[0]), mini_batch_size):
                    ...
                    validate_accuracy = accuracy.eval(feed_dict={
                        ...
                        keep_prob: 1.0})#,
 #                   learning_rate: starting_learning_rate})
                    validation_confusion = validation_confusion + confusion.eval(feed_dict={
                        x: validationBatchData[0][batchStartIndex: batchEndIndex],
                        y_: validationBatchData[1][batchStartIndex:batchEndIndex],
                        keep_prob: 1.0})
                ...
                print("minibatch file "+ str(i) + "/" +str(amountOfMiniBatchFilesToTrain) +  " validation: "  + str(sumOfValidations/amountOfValidations))
                print("minibatch file " +str(i)+ " confusion matrix:\n" + str(validation_confusion))
                sys.stdout.flush()
	...
	amountOfTests = 0
        ...
        fileLocation = "/mnt/snake/snakeNN/snakeNN_data"+ str(6) +"/"
...
#            amountOfTests = 0
            for batchStartIndex in range(0, len(batchData[0]), mini_batch_size):
                ...
                test_accuracy = accuracy.eval(feed_dict={
                    ...
                    keep_prob: 1.0})#,
#                    learning_rate: starting_learning_rate})
                test_confusion = test_confusion + confusion.eval(feed_dict={
                    x: validationBatchData[0][batchStartIndex: batchEndIndex],
                    y_: validationBatchData[1][batchStartIndex:batchEndIndex],
                    keep_prob: 1.0})
