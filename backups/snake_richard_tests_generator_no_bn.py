...
def deepnn(x, keep_prob):
    with tf.name_scope('reshape'):
        x_image = tf.reshape(x, [-1, 52, 52, 4])

    # First convolutional layer - maps one grayscale image to 32 feature maps.
    with tf.name_scope('conv1'):

        W_conv1 = weight_variable([5, 5, 4, 32]) #feature size 5x5 to have 46x46 image after convolution
    ...
    for batchStartIndex in range(0,len(batchData[0]), mini_batch_size):
        ...
        yield [\
            [np.array([\
                ...
                [pixel for pixel in board]]).flatten() for board in batch[0]], \
            batch[1]]
...
#    confusion = np.zeros([3,3])
    with tf.device('/gpu:0'):
        ...
        # Create the model
        x = tf.placeholder(tf.float32, [None, 2704*4], name="x")
